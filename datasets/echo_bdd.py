...
class DetMOTDetection:
    ...
    def _pre_single_frame(self, idx: int, expression_infos):
        ...
        if 'bdd100k' in img_path:
            targets['dataset'] = 'bdd100k'
        else:
    ...
    def __getitem__(self, idx):
        ...
        if 'bdd100k' in img_path:
            ...
            expression_list = os.listdir(osp.join(self.args.rmot_path, 'bdd100k/expressions', video_id))
            expression_random = random.choice(expression_list)
            expression_path = osp.join(self.args.rmot_path, 'bdd100k/expressions', video_id, expression_random)
            with open(expression_path, 'r') as f:
                expression_info = json.load(f)
            sentence = [expression_info['sentence']]
            # to load mp3
            expression_path = osp.join(
                self.args.rmot_path, 'bdd100k/audio-expressions', expression_random.replace('.json', '.mp3')
            )
            try:
                # waveform = librosa.load(expression_path, sr=16000, mono=False)[0]
                waveform, _ = torchaudio.load(expression_path)
                waveform = torch.as_tensor(waveform, dtype=torch.float32).squeeze(0).detach().cpu().numpy()
            except Exception as e:
                raise Exception("{}: open '{}' audio file error.".format(e, expression_path))
            speech = [waveform]
...
def build_dataset2transform(args, image_set):
    ...
    dataset2transform_train = {'bdd100k': kitti_train}
    if image_set == 'train':
