...
from transformers import RobertaModel, RobertaTokenizerFast, AutoFeatureExtractor, HubertModel
from einops import rearrange, repeat

from .backbone import build_backbone
from .deformable_transformer_plus_bi_attn import build_deformable_transformer_bi_attn
from .fusion_helper_mix_fft import VR_Align, agg_ref_feat
...
class MOTR(nn.Module):
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels, criterion, track_embed,
        ...
        self.processor = AutoFeatureExtractor.from_pretrained('weights/hubert-base/', local_files_only=True)
        self.audio_encoder = HubertModel.from_pretrained('weights/hubert-base/', local_files_only=True)

        for p in self.audio_encoder.parameters():
            p.requires_grad_(finetune_audio_encoder)

        self.audio_proj = FeatureResizer(
            input_feat_size=self.audio_encoder.config.hidden_size,
    ...
    def _generate_empty_tracks(self):
        ...
        track_instances.ref_pts = self.transformer.reference_points(
            self.query_embed.weight[:, :dim // 2])  # first hidden_dim for init point prediction
    ...
    def forward_audio(self, audio_queries, device):
        ...
        encoded_audio = self.audio_encoder(
            input_values=processed.input_values, output_hidden_states=True
        ).last_hidden_state
        # Transpose memory because pytorch's attention expects sequence first
        audio_features = self.audio_proj(encoded_audio)  # change text embeddings dim to model dim
        # TODO: Following huggingface
        audio_pad_mask = torch.ones(audio_features.shape[:2], dtype=torch.bool).to(device).detach()  # [B, S]
        # [B, S, 256]
        audio_sentence_features = audio_features
        return audio_features, audio_pad_mask, audio_sentence_features
    ...
    def _forward_single_image(self, samples, track_instances: Instances, sentences):
        ...
        referring_features, referring_mask, _ = self.forward_audio(sentences, src.device)
        ...
        hs, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, referring_dict_features = \
                self.transformer(
                    srcs, masks, pos, track_instances.query_pos, ref_pts=track_instances.ref_pts,
                    referring_dict_features={
                        "hidden": referring_features,
                        "masks": referring_mask
                    }
                )
        ...
        for lvl in range(hs.shape[0]):
            ...
            # [bs, 1, 768]
            referring_pool = agg_ref_feat(
                    referring_dict_features["hidden"], referring_dict_features["masks"]
                ).unsqueeze(1)
            outputs_refer = self.refer_embed[lvl](hs[lvl], referring_pool)
    ...
    def forward(self, data: dict):
        ...
        # TODO: change to referring audio
        sentences = data['speeches']
...
def build(args):
    ...
    transformer = build_deformable_transformer_bi_attn(args)
    ...
    model = MOTR(
        ...
        finetune_audio_encoder=args.finetune_audio_encoder
    )
